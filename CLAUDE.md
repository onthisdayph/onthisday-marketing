# CLAUDE.md — OnThisDay.PH Marketing Website
...
**No test suite exists.** This is a static marketing site; verification is done by running `pnpm build` and visual inspection.
...
- All four frontmatter fields must be present or the build will fail (Zod validation)
...
- **Do not add a JavaScript framework** (React, Vue, Svelte) unless absolutely required for interactivity. The site is intentionally zero-JS static.
